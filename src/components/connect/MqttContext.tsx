...
export const MqttProvider: FC<MqttProviderProps> = ({ children, topic_device }) => {
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe(topic_devices, (err: any) => {
        if (err) {
          console.error("Failed to subscribe to topic", err);
          setSubscriptionError("Failed to subscribe to topic.");
        } else {
          setSubscriptionError(null); // Clear error on success
          console.log(`Successfully subscribed to topic: ${topic_devices}`);
        }
      });
    });
